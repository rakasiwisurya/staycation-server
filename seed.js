...
const seed = async () => {
  ...
  for (const { model, documents } of data) {
    ...
    if (!Model) throw new Error(`Unknown model in seed data: ${model}`);

    await Model.deleteMany({});
    // Users go through create() so the password-hashing save hook runs.
    if (Model === Users) {
      await Model.create(documents);
    } else {
      await Model.insertMany(documents);
    }
    console.log(`  ${model}: ${documents.length} document(s)`);
  }
